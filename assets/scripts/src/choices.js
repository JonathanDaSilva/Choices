...
class Choices {
  constructor(element = "[data-choice]", userConfig = {}) {
    ...
    this.passedElement = isType("String", element)
  ...
  hideDropdown(blurInput = false) {
    ...
    const isFlipped = this.containerOuter.classList.contains(
  ...
  _handleLoadingState(setLoading = true) {
    let placeholderItem = this.itemList.querySelector(
  ...
  _onKeyDown(e) {
    ...
    const onDirectionKey = () => {
      ...
      if (hasActiveDropdown || this.isSelectOneElement) {
        ...
        const skipKey =
          e.metaKey || e.keyCode === pageDownKey || e.keyCode === pageUpKey;
        ...
        if (skipKey) {
          ...
          const currentEl = this.dropdown.querySelector(
            `.${this.config.classNames.highlightedState}`
          );
          if (currentEl) {
  ...
  _onMouseDown(e) {
    ...
    if (this.containerOuter.contains(target) && target !== this.input) {
      ...
      if ((foundTarget = findAncestorByAttrName(target, "data-button"))) {
        ...
        (foundTarget = findAncestorByAttrName(target, "data-choice"))
      ) {
  ...
  _addChoice(
    ...
    const choiceElementId = `${this.baseId}-${
      ...
      containerOuter: direction => {
        return strToEl(`
          <div
            ...
            ${this.isSelectOneElement ? 'tabindex="0"' : ""}
            aria-haspopup="true"
            aria-expanded="false"
            dir="${direction}"
            >
      ...
      choiceList: () => {
        return strToEl(`
          <div
            ...
            dir="ltr"
            role="listbox"
            ${!this.isSelectOneElement ? 'aria-multiselectable="true"' : ""}
            >
          </div>
        `);
      },
      choiceGroup: data => {
        let localClasses = classNames(globalClasses.group, {
